<template>
  <AdminModal
    ...
    dirty-message="Guarda o descarta los cambios antes de cerrar."
    @close="emit('close')"
  >
      ...
      <section class="student-workspace">
        <nav class="student-tabs" aria-label="Secciones de la ficha">
          ...
          <button type="button" :class="{ active: activeTab === 'familia' }" @click="activeTab = 'familia'">
            <FamilyPersonasIcon name="people" /> Editar familia
          </button>
        </nav>
        ...
        <div v-if="activeTab === 'consulta'" class="consultation-view">
          <section class="consultation-hero" :data-level="levelKey">
            <div>
              <p class="mkt-eyebrow">Ficha</p>
              <h3>{{ student.fullName }}</h3>
              <p>{{ student.nivel || 'Nivel escolar' }} · {{ gradeGroup }} · {{ cicloLabel }}</p>
              <button class="edit-family-shortcut" type="button" @click="activeTab = 'familia'">Editar familia</button>
            </div>
        ...
        <form v-else class="family-form" @submit.prevent="save">
          ...
          <div v-if="saveState !== 'idle'" class="save-state" :data-state="saveState">
            <span />{{ saveState === 'saving' ? 'Guardando…' : saveState === 'saved' ? 'Información actualizada' : 'No se pudo guardar' }}
          </div>
