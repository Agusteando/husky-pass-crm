<template>
  <section class="room-users" data-product-area="daycare" data-product-screen="room-users">
    ...
    <template v-else-if="overview">
      ...
      <section class="signal-grid" aria-label="Estado de usuarios">
        ...
        <button type="button" class="merge-all-signal" :disabled="!overview.duplicates.length || actionBusy" @click="openMergeAll">
          <span class="signal-icon blue"><FamilyPersonasIcon name="sparkles" /></span>
          <span><small>Consolidación</small><strong>{{ overview.duplicates.length ? 'Lista' : 'Limpia' }}</strong></span>
        ...
        <aside class="member-detail" data-product-panel="user-detail">
          <template v-if="selectedMember">
            ...
            <section class="room-flow">
              <header><p class="eyebrow">Sala actual</p><strong>{{ selectedMember.salaName || 'Sin sala' }}</strong></header>
              <div class="flow-track">
                <button type="button" :disabled="!previousRoom || actionBusy" @click="moveMemberTo(previousRoom?.id)">
                  <FamilyPersonasIcon name="arrow" />
                  <span><small>Regresar</small><strong>{{ previousRoom?.sala || '—' }}</strong></span>
                </button>
                <span class="flow-current"><i>{{ currentRoomPosition }}</i><strong>{{ selectedMember.salaName || '—' }}</strong></span>
                <button type="button" :disabled="!nextRoom || actionBusy" @click="moveMemberTo(nextRoom?.id)">
                  <span><small>Promover</small><strong>{{ nextRoom?.sala || '—' }}</strong></span>
                  <FamilyPersonasIcon name="arrow" />
                </button>
              </div>
              <div class="direct-move">
                <select v-model.number="detailTargetSalaId" aria-label="Mover a sala">
                  <option :value="0" disabled>Sala destino</option>
                  <option v-for="sala in overview.salas" :key="sala.id" :value="sala.id">{{ sala.sala }}</option>
                </select>
                <button type="button" :disabled="!detailTargetSalaId || detailTargetSalaId === selectedMember.salaId || actionBusy" @click="moveMemberTo(detailTargetSalaId)">Mover</button>
              </div>
            </section>
...
.room-flow { border-top: 1px solid var(--line); display: grid; gap: 12px; padding: 17px 14px; }
.room-flow header { align-items: end; display: flex; justify-content: space-between; }
.room-flow header strong { color: var(--ink); font-size: .82rem; }
.flow-track { align-items: stretch; display: grid; gap: 6px; grid-template-columns: minmax(0,1fr) 58px minmax(0,1fr); }
.flow-track button { align-items: center; background: #f4f7ef; border: 1px solid rgba(66,104,49,.11); border-radius: 14px; color: #456d2d; display: grid; gap: 5px; grid-template-columns: 18px minmax(0,1fr); min-height: 58px; padding: 8px; text-align: left; }
.flow-track button:last-child { grid-template-columns: minmax(0,1fr) 18px; text-align: right; }
.flow-track button:first-child :deep(.pa-icon) { transform: rotate(180deg); }
.flow-track button:disabled { opacity: .42; }
.flow-track button span { display: grid; min-width: 0; }
.flow-track button small { color: #899384; font-size: .57rem; font-weight: 800; }
.flow-track button strong { font-size: .66rem; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.flow-current { align-items: center; background: linear-gradient(135deg,#4c7b2e,#8dab3d); border-radius: 15px; color: #fff; display: grid; justify-items: center; padding: 7px 3px; }
.flow-current i { font-size: .58rem; font-style: normal; opacity: .74; }
.flow-current strong { font-size: .62rem; max-width: 52px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.direct-move { display: grid; gap: 7px; grid-template-columns: minmax(0,1fr) auto; }
.direct-move select { background: #fff; border: 1px solid var(--line); border-radius: 13px; color: var(--ink); min-height: 41px; padding: 0 10px; }
.direct-move button,
...
.direct-move button:disabled { opacity: .45; }
