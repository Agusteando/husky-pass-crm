    ...
    <header class="users-hero">
      <div class="hero-copy">
        ...
        <div class="hero-metrics">
          <span><strong>{{ overview?.stats.total || 0 }}</strong> usuarios</span>
          <span><strong>{{ overview?.salas.length || 0 }}</strong> salas</span>
          <span><strong>{{ overview?.unidad || '—' }}</strong></span>
        </div>
      </div>

      <div class="hero-controls">
        <label v-if="(overview?.unidades.length || 0) > 1" class="control-card">
          <span>Unidad</span>
        ...
        <div v-else class="control-card unit-card">
          <span>Unidad</span>
          <strong>{{ overview?.unidad || selectedUnidad || '—' }}</strong>
        </div>
        ...
        <aside class="member-detail" data-product-panel="user-detail">
          <template v-if="selectedMember">
            ...
            <section class="room-flow">
              <header>
                <div><p class="eyebrow">Sala actual</p><strong>{{ selectedMember.salaName }}</strong></div>
              ...
              <div class="room-path" role="list" aria-label="Recorrido de salas">
                <button
                  ...
                  :disabled="actionBusy"
                  @click="selectTargetRoom(sala.id)"
                >
              ...
              <Transition name="room-transfer">
                <div v-if="detailTargetRoom && detailTargetRoom.id !== selectedMember.salaId" class="room-transfer">
                  <span class="transfer-room current">{{ selectedMember.salaName }}</span>
                  <FamilyPersonasIcon name="arrow" />
                  <span class="transfer-room target">{{ detailTargetRoom.sala }}</span>
                  <button type="button" :disabled="actionBusy" :aria-label="`Mover a ${detailTargetRoom.sala}`" @click="moveMemberTo(detailTargetRoom.id)">
...
import type { DaycareRoomManagementOverview, DaycareRoomMember, Sala } from '~/types/daycare'
...
watch(overview, (value) => {
  if (!value) return
  if (selectedUnidad.value !== value.unidad) selectedUnidad.value = value.unidad
  if (!['all', ...value.salas.map((sala) => String(sala.id))].includes(selectedRoomKey.value)) selectedRoomKey.value = 'all'
...
watch([selectedUnidad, selectedRoomKey, search, selectedMemberId], () => syncQuery())

const filteredMembers = computed(() => {
  const members = overview.value?.members || []
  const needle = search.value.trim().toLowerCase()
  return members.filter((member) => {
    if (selectedRoomKey.value !== 'all' && String(member.salaId || '') !== selectedRoomKey.value) return false
    if (!needle) return true
    return `${member.nombre_nino || ''} ${member.email || ''} ${member.username || ''} ${member.salaName || ''}`.toLowerCase().includes(needle)
  })
})

const selectedMember = computed(() => filteredMembers.value.find((member) => member.id === selectedMemberId.value) || filteredMembers.value[0] || null)
const selectedRoom = computed(() => overview.value?.salas.find((sala) => String(sala.id) === selectedRoomKey.value) || null)
const roomHeading = computed(() => selectedRoom.value?.sala || 'Todas las salas')
...
const nextRoom = computed(() => currentRoomIndex.value >= 0 ? overview.value?.salas[currentRoomIndex.value + 1] || null : null)
...
watch(selectedMember, (member) => {
  ...
  if (member && selectedMemberId.value !== member.id) selectedMemberId.value = member.id
})
...
function selectRoom(value: string) {
  ...
  selectedIds.value = []
  selectedMemberId.value = filteredMembers.value[0]?.id || null
}
...
function selectTargetRoom(targetSalaId: number) {
  if (actionBusy.value) return
  if (targetSalaId === selectedMember.value?.salaId) {
    moveMemberTo(targetSalaId, true)
    return
  }
  detailTargetSalaId.value = targetSalaId
}

async function moveMemberTo(targetSalaId?: number | null, force = false) {
  if (actionBusy.value || !selectedMember.value?.id || !targetSalaId) return
  if (!force && targetSalaId === selectedMember.value.salaId) return
...
async function moveMembers(userIds: number[], targetSalaId: number) {
  ...
  try {
    ...
    if (userIds.length === 1 && result.userIds[0]) selectedMemberId.value = result.userIds[0]
...
.unit-card strong { color: #fff; font-size: .9rem; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
...
.bulk-bar { align-items: center; background: linear-gradient(135deg,#314f24,#5f8737); border-radius: 17px; color: #fff; display: grid; gap: 8px; grid-template-columns: 30px auto minmax(150px,1fr) auto 34px; margin: 12px 14px 0; padding: 9px 10px; }
...
.bulk-bar > button:disabled { opacity: .48; }
.clear-selection { align-items: center; background: transparent; border: 0; color: #fff; display: flex; justify-content: center; }
.table-head { color: #8a9386; display: grid; font-size: .62rem; font-weight: 900; gap: 10px; grid-template-columns: 26px minmax(220px,1fr) minmax(110px,.62fr) 30px; letter-spacing: .08em; padding: 13px 16px 8px; text-transform: uppercase; }
.member-list { align-content: start; display: grid; gap: 5px; overflow: auto; padding: 5px 9px 12px; }
.member-row { align-items: center; border: 1px solid transparent; border-radius: 18px; cursor: pointer; display: grid; gap: 10px; grid-template-columns: 26px 42px minmax(160px,1fr) minmax(110px,.62fr) 30px; min-height: 66px; padding: 7px; }
...
.room-pill { align-items: center; background: #edf5e5; border-radius: 999px; color: #49752f; display: inline-flex; font-size: .64rem; font-weight: 900; gap: 5px; justify-self: start; max-width: 100%; overflow: hidden; padding: 6px 8px; text-overflow: ellipsis; white-space: nowrap; }
.room-pill :deep(.pa-icon) { height: .85rem; width: .85rem; }
.member-row > button { align-items: center; background: transparent; border: 0; color: #71806b; display: flex; justify-content: center; }

.member-detail { gap: 0; }
.detail-head { align-items: center; border-bottom: 1px solid var(--line); display: grid; gap: 11px; grid-template-columns: 54px minmax(0,1fr); padding: 19px; }
...
.room-transfer button { align-items: center; background: #fff; border: 0; border-radius: 13px; color: #416a28; display: inline-flex; height: 38px; justify-content: center; padding: 0; width: 38px; }
...
.surface-message { background: #eff7e8; border: 1px solid rgba(87,139,38,.16); border-radius: 17px; color: #456d2d; font-size: .76rem; font-weight: 900; margin: 0; padding: 12px 14px; }
.surface-message.error { background: #fff0ed; border-color: rgba(186,77,61,.17); color: #a44d41; }
...
@media (max-width: 720px) {
  ...
  .member-row .room-pill { grid-column: 3; }
  ...
  .room-path button { flex-basis: 82px; }
}
</style>
