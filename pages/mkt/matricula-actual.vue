<template>
  <section class="mkt-page enrollment-page" data-product-area="mkt" data-product-screen="current-enrollment">
    ...
    <section v-else-if="optionsError" class="mkt-panel page-state" data-state="error">
      <FamilyPersonasIcon name="alert" />
      <div><h2>Aurora no respondió</h2><p>{{ optionErrorMessage }}</p></div>
      <button class="mkt-btn" type="button" @click="refreshOptions">Reintentar</button>
    </section>

    <section v-else-if="!options?.planteles.length" class="mkt-panel page-state no-planteles">
      <img src="/personas-autorizadas/ambassadors/primaria-brave.png" alt="" aria-hidden="true" />
      <div><p class="mkt-eyebrow">Planteles</p><h2>Sin planteles asignados</h2></div>
    </section>
        ...
        <section class="overview-layout">
          <article class="grade-matrix mkt-panel">
            ...
            <div v-else class="matrix-empty">
              ...
              <h3>Sin matrícula en este ciclo</h3>
            </div>
          </article>
...
const optionErrorMessage = computed(() => optionsFetchError.value?.data?.message || optionsFetchError.value?.message || 'No fue posible abrir la conexión.')
