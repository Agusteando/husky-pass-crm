...
type AuroraAccount = {
  ...
  conceptos?: Array<{
    ...
    recibos?: number[]
    servicio?: { clave?: string | null; nombre?: string | null; imagen?: string | null; estado?: string | null } | null
  }>
  recibos?: Array<{
    folio?: number | null
...
function config() {
  ...
  return {
    baseURL: String(runtime.aurora?.apiBaseUrl || process.env.AURORA_API_BASE_URL || process.env.HUSKY_PASS_AURORA_API_BASE_URL || '').trim().replace(/\/+$/, ''),
    token: String(runtime.aurora?.apiToken || process.env.AURORA_API_TOKEN || process.env.HUSKY_PASS_AURORA_API_TOKEN || '').trim(),
    timeout: Number(runtime.aurora?.timeoutMs || process.env.AURORA_API_TIMEOUT_MS || process.env.HUSKY_PASS_AURORA_TIMEOUT_MS || 10000),
    ciclo: String(runtime.aurora?.ciclo || process.env.AURORA_CICLO || process.env.HUSKY_PASS_AURORA_CICLO || deriveCurrentCycle()).trim()
  }
}
...
function paymentCategory(value: unknown): PaymentCategory {
  const normalized = String(value || '').trim().toLowerCase()
  if (normalized === 'taller_servicio' || normalized === 'taller') return 'taller'
...
function mapService(raw: NonNullable<AuroraAccount['servicios']>[number] | NonNullable<NonNullable<AuroraAccount['conceptos']>[number]['servicio']>): PaymentServiceBadge | null {
  ...
  return {
    ...
    folios: 'folios' in raw && Array.isArray(raw.folios) ? raw.folios.map(Number).filter(Boolean) : []
  }
}

function mapReceipt(raw: NonNullable<AuroraAccount['recibos']>[number]): PaymentReceipt | null {
...
function mapConcept(raw: NonNullable<AuroraAccount['conceptos']>[number]): PaymentItem {
  ...
  const title = String(raw.titulo || raw.conceptoNombre || 'Concepto escolar').trim()
  const paidAmount = toNumber(raw.pagado)
  return {
    id: String(raw.id || `${raw.documento || 'concepto'}-${raw.periodo || title}`),
    title,
    description: String(raw.descripcion || service?.nombre || (status === 'paid' ? 'Cubierto en Aurora.' : 'Cargo registrado en Aurora.')).trim(),
    ...
    period: raw.periodo || null,
    receiptFolios: Array.isArray(raw.recibos) ? raw.recibos.map(Number).filter(Boolean) : [],
    service,
    actionLabel: status === 'paid'
      ? (raw.recibos?.length ? `Recibo ${raw.recibos.join(', ')}` : 'Pagado')
      : balance > 0 ? 'Pendiente en Aurora' : 'Registrado'
  }
}

async function fetchAuroraAccount(matricula: string): Promise<AuroraAccount> {
  const aurora = config()
  if (!aurora.baseURL || !aurora.token) {
    const missing = !aurora.baseURL ? 'baseURL' : 'token'
    throw Object.assign(new Error('Aurora no está configurado para Husky Pass.'), { code: 'AURORA_CONFIG_MISSING', missing })
  }

  return await $fetch<AuroraAccount>('/api/external/v1/husky-pass/account', {
    baseURL: aurora.baseURL,
    timeout: Number.isFinite(aurora.timeout) && aurora.timeout > 0 ? aurora.timeout : 10000,
    headers: {
...
function childFromAurora(student: AuthorizedChild | null, account: AuroraAccount): AuthorizedChild | null {
  if (!account.alumno) return student
  const [paternoA, maternoA, ...rest] = String(account.alumno.nombre || '').split(' ').filter(Boolean)
  return {
    ...student,
    paternoA: student?.paternoA || paternoA || null,
    maternoA: student?.maternoA || maternoA || null,
    nombreA: student?.nombreA || (rest.length ? rest.join(' ') : account.alumno.nombre) || null,
    ...
    matricula: normalizeMatricula(account.matricula || student?.matricula) || null,
...
export async function getFamilyPayments(user: AppSessionUser): Promise<FamilyPaymentsResponse> {
  ...
  try {
    const account = await fetchAuroraAccount(matricula)
    const items = (account.conceptos || []).map(mapConcept)
    const receipts = (account.recibos || []).map(mapReceipt).filter(Boolean) as PaymentReceipt[]
    const services = (account.servicios || []).map(mapService).filter(Boolean) as PaymentServiceBadge[]
    const responseStudent = childFromAurora(student, account)
    const sourceLabel = account.fuente?.agentId ? `Aurora · ${account.fuente.agentId}` : 'Aurora'
    ...
    return {
      ...
      summary: {
        ...
        overdueCount: Number(account.conteos?.vencidos || items.filter((item) => item.status === 'overdue').length),
        pendingCount: Number(account.conteos?.pendientes || items.filter((item) => item.status === 'pending').length),
        ...
        currency: 'MXN',
        cycleLabel: account.ciclo?.nombre || null,
      ...
      state: items.length || receipts.length || services.length ? 'ready' : 'empty',
      message: items.length || receipts.length || services.length ? undefined : 'Aurora no tiene cargos ni recibos publicados para esta matrícula.'
    }
  } catch (error: unknown) {
    const fetchError = error as FetchError & { code?: string; missing?: string }
