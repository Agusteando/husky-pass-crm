...
export default defineEventHandler(async (event) => {
  ...
  if (!match) throw createError({ statusCode: 415, statusMessage: 'La imagen procesada debe ser PNG Base64.' })

  const buffer = Buffer.from(match[1], 'base64')
  if (!buffer.length || buffer.length > 1024 * 1024 * 2) {
    throw createError({ statusCode: 413, statusMessage: 'La imagen procesada excede el tamano permitido.' })
  }
